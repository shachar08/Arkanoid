...
public class FourthLevel implements LevelInformation {
    ...
    public java.util.List<Block> blocks() {
        ...
        Color[] arr = {Color.GRAY, Color.RED, Color.YELLOW, Color.GREEN, Color.WHITE, Color.PINK, Color.CYAN};
        Point p = new Point(730, 100);
        for (int i = 0; i < 7; i++) { // create all the other blocks for the game
            for (int j = 0; j < 15; j++) {
                blocks.add(new Block(new Rectangle(new Point(p.getX(), p.getY()), 51, 20), arr[i]));
                p = new Point(p.getX() - 50 - 2.0 / 3, p.getY());
            }
